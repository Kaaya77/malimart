...
export const SellerMessages = ({ userId, selectedChatUser, setSelectedChatUser, products, initialProductId, initialOrderId, initialChatUser }: { userId: string, selectedChatUser: string | null, setSelectedChatUser: (uid: string | null) => void, products: Product[], initialProductId?: string | null, initialOrderId?: string | null, initialChatUser?: string | null }) => {
 ...
 <div className={`p-4 text-[13px] shadow-sm relative leading-relaxed ${isMe ? 'bg-primary text-background dark:bg-background dark:text-foreground rounded-2xl rounded-tr-sm' : 'bg-card text-foreground border border-foreground/10 rounded-2xl rounded-tl-sm'}`}>
 {c.attachment_url && (
 <div className="mb-3">
 ...
 <div className="bg-card border border-foreground/10 rounded-2xl rounded-tl-sm p-4 flex gap-1.5 items-center h-12 shadow-sm">
 ...
 className={`px-4 py-2 text-[9px] uppercase tracking-[0.2em] transition-all border ${magicTone === tone ? 'bg-primary text-background dark:bg-background dark:text-foreground border-foreground dark:border-background' : 'bg-transparent text-foreground/60 border-foreground/20 hover:border-foreground hover:text-foreground'}`}
 >
 {tone}
 </button>
 ))}
 ...
 <form onSubmit={handleSend} className="flex gap-3 items-end bg-card p-2 rounded-2xl border border-foreground/10 shadow-sm focus-within:border-foreground/30 dark:focus-within:border-background/30 transition-all">
 ...
 className="p-2 rounded-lg bg-transparent text-foreground/40 hover:bg-foreground/[0.04] hover:text-foreground transition-all"
 title="Attach File"
 disabled={isUploading}
 >
 {isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Paperclip className="w-4 h-4 stroke-[1]" />}
 </button>
 <button type="button" onClick={() => magicMode ? handleMagicPolish() : setMagicMode(true)} className={`p-2 rounded-lg transition-all ${magicMode ? 'bg-primary text-background dark:bg-background dark:text-foreground' : 'bg-transparent text-foreground/40 hover:bg-foreground/[0.04] hover:text-foreground'}`} title="Magic Compose">
 ...
 className="w-full h-12 bg-transparent border border-foreground/10 px-4 text-sm focus:outline-none focus:border-foreground transition-colors appearance-none"
 >
