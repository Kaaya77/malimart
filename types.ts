...
// ═══════════════════════════════════════════════════════════════════════════
// COMPANION — the date/relationship concierge domain.
// Single-player: `user_id` is always the planner, never the partner.
// See supabase/migrations/20260901120000_companion_core.sql.
// ═══════════════════════════════════════════════════════════════════════════
...
export interface Partner {
    ...
    ritual_morning: boolean;
    ritual_evening: boolean;
    date_cadence_days: number;
    is_active: boolean;
    created_at: string;
    updated_at: string;
}

export type PartnerNoteKind = 'fact' | 'wish' | 'moment';
...
export type SpotCategory =
    | 'restaurant' | 'breakfast' | 'sunset' | 'nightlife'
    | 'beach' | 'activity' | 'gift' | 'movie' | 'event';
...
/**
 * Nudges are DERIVED client-side from occasions/promises/date_log — never
 * stored. `key` is deterministic and stable for a given nudge on a given day,
 * so dismissals persist across devices without a generator job.
 *
 * `effort` is the ladder from the product brief: the app must always be able
 * to offer something that costs 30 seconds and nothing.
 */
export type NudgeEffort = 'tiny' | 'small' | 'evening' | 'big';
export type NudgeKind = 'ritual' | 'occasion' | 'promise' | 'drift';
...
export interface Nudge {
    ...
    title: string;
    body: string;
    /** Sort priority — higher first. Never shown to the user as a score. */
    weight: number;
    /** Optional deep link into the app (a spot category, the partner page…). */
    href?: string;
    /** Set for promise nudges so "Done" can close the promise itself. */
    promiseId?: string;
}
