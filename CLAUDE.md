...
- **Page gutter is `px-4 md:px-8`** — every page-level container and the Navbar. It had drifted to `md:px-6` (Navbar, Cart, dashboards) and `px-5` (home sections), so the header never lined up with the content under it and homepage sections stepped in/out by 4px. Modal-internal padding is separate and may differ.
- **Never paint a LARGE surface with `bg-foreground text-background`.** That pair INVERTS with the theme — near-black in light mode, cream in dark — so a hero/banner becomes a glaring cream slab in dark mode. It is fine for small "selected" chips where inversion is the point. For a banner that should stay dark in both themes, pin it: `bg-stone-900 text-stone-50` (stone-900 is #1c1917, identical to what `--foreground` resolves to in light mode, so light mode is unchanged). Fixed in CategoriesPage's hero and ExploreBanner.
- **Dark mode is CLASS-based, not OS-based.** `src/index.css` declares `@custom-variant dark (&:where(.dark, .dark *))`. Never remove it: Tailwind v4 defaults `dark:` to `prefers-color-scheme`, which would silently split the app into two half-themes (tokens follow the toggle, `dark:` utilities follow the OS). Also never pair a fixed surface with a themed ink (`bg-white/80 text-foreground/60`) — `--foreground` inverts between themes, so that vanishes in one of them.
