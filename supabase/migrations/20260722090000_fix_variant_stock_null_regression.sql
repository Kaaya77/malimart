-- 20260722090000_fix_variant_stock_null_regression.sql
-- Fixes a regression introduced in 20260721160000: the atomic stock-decrement
-- guard added `WHERE stock >= quantity` to close an oversell race, but only
-- applied the NULL-safe form (`stock IS NULL OR stock >= quantity`) to the
-- plain-product branch. The variant branch was left as a bare
-- `stock >= item.quantity`, which evaluates to NULL (not TRUE) for any
-- variant with untracked/NULL stock — silently failing every order for that
-- variant with "Insufficient stock" even though NULL stock means unlimited,
-- exactly as the pre-check loop above it already treats it. No live variant
-- currently has NULL stock, but the bug is real for the first one that does.
