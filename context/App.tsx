...
const PAGE_TITLES: Record<string, string> = {
  ...
  '/shop': 'Shop — MaliMart',
  '/cart': 'Your Basket — MaliMart',
  '/checkout': 'Checkout — MaliMart',
  '/categories': 'Categories — MaliMart',
  '/buyer': 'My Account — MaliMart',
  '/seller': 'Seller Hub — MaliMart',
  '/admin': 'Admin — MaliMart',
  '/messages': 'Messages — MaliMart',
  '/notifications': 'Notifications — MaliMart',
  '/auth': 'Sign In — MaliMart',
  '/auth/reset': 'Reset Password — MaliMart',
  '/terms': 'Terms of Service — MaliMart',
  '/privacy': 'Privacy Policy — MaliMart',
  '/returns': 'Returns & Refunds — MaliMart',
  '/seller-agreement': 'Seller Agreement — MaliMart',
  '/contact': 'Contact Us — MaliMart',
};
...
const AppContent = () => {
  ...
  // Route-driven page titles — better tabs, history, and link shares
  useEffect(() => {
    const base = location.pathname.split('/').slice(0, 3).join('/');
    document.title = PAGE_TITLES[location.pathname]
      || PAGE_TITLES[base]
      || (location.pathname.startsWith('/product/') ? 'Product — MaliMart'
        : location.pathname.startsWith('/store/') ? 'Store — MaliMart'
        : 'MaliMart — Tanzania\u2019s Marketplace');
  }, [location.pathname]);
