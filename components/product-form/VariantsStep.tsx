...
// Visible active-cell indicator for the inline matrix inputs.
const CELL_FOCUS = 'focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500/50';
...
export const VariantsStep = () => {
 ...
 <Button size="sm" variant="outline" onClick={handleSuggestAttributes} isLoading={aiLoading} className="text-[10px] uppercase tracking-widest h-10 bg-transparent border-foreground/10"><Wand2 className="w-3 h-3 mr-2"/> AI Suggest</Button>
 ...
 className={`h-10 px-3 text-[10px] uppercase tracking-widest font-bold border border-foreground/10 rounded-xl hover:bg-foreground/[0.06] transition-colors text-foreground/70 hover:text-foreground active:scale-95 ${FOCUS_RING}`}
 >
 + {attr}
 </button>
 ))}
 ...
 <button onClick={() => setAttributes(attributes.filter((_, i) => i !== idx))} aria-label="Remove attribute" className={`absolute top-3 right-3 w-10 h-10 flex items-center justify-center rounded-xl opacity-40 hover:opacity-100 hover:bg-foreground/[0.05] transition-all ${FOCUS_RING}`}><Trash2 className="w-4 h-4"/></button>
 ...
 <div className="flex flex-col md:flex-row justify-between items-center bg-foreground/[0.02] p-4 border border-foreground/10 rounded-2xl gap-4">
 <h3 className="text-[10px] uppercase tracking-widest font-bold text-foreground shrink-0">Variant Matrix ({variants.length})</h3>
 <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
 <div className="flex items-center gap-2 p-1.5 bg-foreground/[0.03] border border-foreground/10 rounded-2xl">
 <Input placeholder="Bulk Price.." aria-label="Bulk price" className={`h-10 w-24 text-[10px] bg-transparent border-none ${CELL_FOCUS}`} value={bulkPrice} onChange={(e: any) => setBulkPrice(e.target.value)} />
 <Input placeholder="Bulk Stock.." aria-label="Bulk stock" className={`h-10 w-24 text-[10px] bg-transparent border-none ${CELL_FOCUS}`} value={bulkStock} onChange={(e: any) => setBulkStock(e.target.value)} />
 <button onClick={handleBulkApply} className={`h-10 px-4 rounded-xl bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-600 active:scale-95 transition-all ${FOCUS_RING}`}>Apply All</button>
 </div>
 <button onClick={handleAutoSkuVariants} className={`h-11 px-5 rounded-xl bg-transparent border border-foreground/10 text-foreground text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-foreground/[0.04] active:scale-95 transition-all ${FOCUS_RING}`}><Wand2 className="w-4 h-4"/> Auto-SKU</button>
 </div>
 </div>
 ...
 <button onClick={() => document.getElementById(`var-img-${i}`)?.click()} aria-label="Upload variant image" className={`w-7 h-7 flex items-center justify-center bg-background text-foreground hover:opacity-80 rounded-md ${FOCUS_RING}`}><Upload className="w-3 h-3"/></button>
 <button onClick={() => handleRefurbishVariant(i)} aria-label="Enhance variant image with AI" className={`w-7 h-7 flex items-center justify-center bg-emerald-500 text-white hover:bg-emerald-600 rounded-md ${FOCUS_RING}`} disabled={refurbishingIdx === i}>{refurbishingIdx === i ? <Loader2 className="w-3 h-3 animate-spin"/> : <Sparkles className="w-3 h-3"/>}</button>
 {v.image_url && <button onClick={() => downloadImage(v.image_url!, `variant-${i}.png`)} aria-label="Download variant image" className={`w-7 h-7 flex items-center justify-center bg-background text-foreground hover:opacity-80 rounded-md ${FOCUS_RING}`}><Download className="w-3 h-3"/></button>}
 </div>
 ...
 <span className="text-[10px] uppercase opacity-40 mb-0.5">{key}</span>
 <span className="px-2.5 py-1.5 rounded-lg bg-foreground/[0.05] text-[10px] text-foreground border border-foreground/10 uppercase tracking-widest">{String(val)}</span>
 </div>
 ))}
 </div>
 </td>
 <td className="p-4"><Input type="number" aria-label="Variant price" className={`h-10 text-xs bg-transparent border-foreground/10 ${CELL_FOCUS}`} value={v.base_price || 0} onChange={e => { const n = [...variants]; n[i].base_price = Number(e.target.value); setVariants(n); }} /></td>
 <td className="p-4"><Input type="number" aria-label="Variant stock" className={`h-10 text-xs bg-transparent border-foreground/10 text-center ${CELL_FOCUS}`} value={v.stock || 0} onChange={e => { const n = [...variants]; n[i].stock = Number(e.target.value); setVariants(n); }} /></td>
 <td className="p-4"><Input type="text" aria-label="Variant SKU" className={`h-10 text-[10px] font-mono font-black bg-foreground/[0.04] border-foreground/10 uppercase ${CELL_FOCUS}`} value={v.sku || ''} onChange={e => { const n = [...variants]; n[i].sku = e.target.value.toUpperCase(); setVariants(n); }} /></td>
 <td className="p-4 text-right"><button onClick={() => setVariants(variants.filter((_, idx) => idx !== i))} aria-label="Remove variant" className={`w-10 h-10 inline-flex items-center justify-center rounded-xl text-foreground/30 hover:text-red-500 hover:bg-red-500/[0.06] transition-all opacity-0 group-hover:opacity-100 focus-visible:opacity-100 ${FOCUS_RING}`}><Trash2 className="w-4 h-4"/></button></td>
 </tr>
 ))}
 </tbody>
 </table>
