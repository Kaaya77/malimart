...
export const SellerInventory = ({ products: initialProducts, userId, refresh, onCreatePromo }: { products: Product[], userId: string, refresh: () => void, onCreatePromo: (p: Product) => void }) => {
 ...
 const [rpcTotals, setRpcTotals] = useState<any>(null);
 const PAGE_SIZE = 50;
 ...
 // Analytics — prefer RPC totals (full dataset), fall back to page slice
 const stats = useMemo(() => {
  ...
  if (rpcTotals) {
   return {
    totalStock,
    value:      Number(rpcTotals.inventory_value) || 0,
    lowStock:   Number(rpcTotals.low_stock)       || 0,
    totalVariants,
    outOfStock: Number(rpcTotals.out_of_stock)    || 0,
   };
  }
  return {
   totalStock,
   value:      products.reduce((acc, p) => acc + (p.price * p.stock), 0),
   lowStock:   products.filter(p => p.stock < (p.low_stock_threshold || 5)).length,
   totalVariants,
   outOfStock: 0,
  };
 }, [products, rpcTotals]);

 // Data Fetching — single RPC replaces products+variants waterfall
 const fetchInventory = useCallback(async () => {
  ...
  try {
   ...
   let result = (data?.products ?? []) as Product[];
   if (advFilters.priceMin) result = result.filter((p: Product) => p.price >= Number(advFilters.priceMin));
   if (advFilters.priceMax) result = result.filter((p: Product) => p.price <= Number(advFilters.priceMax));
   if (advFilters.isBoosted) result = result.filter((p: Product) => p.is_boosted);
   if (advFilters.hasVariants) result = result.filter((p: Product) => (p.variants?.length || 0) > 0);
   setProducts(result);
   setTotalCount(data?.pagination?.matched ?? data?.totals?.total ?? 0);
   if (data?.totals) setRpcTotals(data.totals);
  } catch (e: any) {
   addToast("Error fetching inventory", "error");
   console.error('[SellerInventory]', e?.message);
  }
  setLoading(false);
 }, [userId, page, debouncedSearch, category, status, sort, advFilters]);

 useEffect(() => { setPage(0); }, [debouncedSearch, category, status]);
