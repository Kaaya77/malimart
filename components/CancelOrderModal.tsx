            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <motion.div 
                        ...
                        className="bg-white dark:bg-[#0a0a0a] rounded-3xl p-8 max-w-md w-full shadow-2xl border border-slate-100 dark:border-white/5 relative"
                    >
                    ...
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Cancel Order</h2>
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                        {reasons.map((reason) => (
                            <label key={reason} className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedReason === reason ? 'border-foreground dark:border-background bg-slate-50 dark:bg-white/5' : 'border-slate-100 dark:border-white/5 hover:border-slate-200 dark:hover:border-white/10'}`}>
                                <input 
                                    type="radio" 
                                    name="cancelReason" 
                                    value={reason} 
                    ...
                    {selectedReason === 'Other' && (
                        <div className="mb-6">
                            <Label className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2 block">Please specify</Label>
                            <Input 
                                value={customReason} 
                                onChange={(e: any) => setCustomReason(e.target.value)} 
                                placeholder="Enter your reason..."
                                className="h-12 rounded-2xl bg-slate-50 dark:bg-white/5 border-none"
                            />
                        </div>
                    )}
