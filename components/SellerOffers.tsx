        ...
        <div className={`h-32 bg-gradient-to-r ${getPreviewGradient()} p-6 relative overflow-hidden transition-colors duration-500`}>
            ...
            <div className="relative z-10 flex justify-between items-start">
                <div className="flex items-center gap-2">
                    <div className="w-10 h-10 rounded-none bg-white/20 backdrop-blur-md p-0.5 shadow-lg flex items-center justify-center text-white">
                    ...
                    offers.map(offer => (
                        <div key={offer.id} className={`relative group p-6 rounded-none border shadow-none transition-all ${offer.status === 'active' ? 'bg-background dark:bg-background border-foreground/10 dark:border-background/10 hover:border-foreground/30 dark:hover:border-background/30' : 'bg-primary/5 dark:bg-background/5 border-transparent opacity-75'}`}>
                            ...
                            {!offer.is_auto_apply ? (
                                ...
                                <div className="p-3 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-none border border-emerald-100/50 dark:border-emerald-900/20 text-center">
                            ...
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-background dark:bg-background p-1 rounded-none shadow-sm border border-foreground/10 dark:border-background/10">
                                ...
                                <button onClick={() => confirmDelete(offer.id)} className="p-2 text-red-500 hover:bg-red-50/50 rounded-none transition-colors" title="Delete">
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="p-6 bg-primary/5 dark:bg-background/5 rounded-none border border-foreground/10 dark:border-background/10 space-y-6">
                                        ...
                                        {formData.campaign_type === 'discount' && (
                                            <div className="grid grid-cols-2 gap-6 animate-in fade-in">
                                                ...
                                                <div>
                                                    <Label>Value</Label>
                                                    <Input type="number" placeholder="0" value={Number.isNaN(formData.value) ? '' : (formData.value ?? '')} onChange={(e:any) => setFormData({...formData, value: e.target.value === '' ? null : Number(e.target.value)})} className="h-14 font-serif text-lg rounded-none" />
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {formData.campaign_mode === 'flash' && (
                                            <div className="animate-in fade-in">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div><Label>Start</Label><Input type="date" value={formData.start_date || ''} onChange={(e:any) => setFormData({...formData, start_date: e.target.value})} className="h-12 rounded-none" /></div>
                                                    <div><Label>End</Label><Input type="date" value={formData.end_date || ''} onChange={(e:any) => setFormData({...formData, end_date: e.target.value})} className="h-12 rounded-none" /></div>
                                                </div>
                                            </div>
                                        )}

                                        <div className="grid grid-cols-2 gap-6">
                                            <div><Label>Min Spend</Label><Input type="number" placeholder="0" value={Number.isNaN(formData.min_order_value) ? '' : (formData.min_order_value ?? '')} onChange={(e:any) => setFormData({...formData, min_order_value: e.target.value === '' ? null : Number(e.target.value)})} className="h-12 rounded-none" /></div>
                                            <div><Label>Usage Limit</Label><Input type="number" placeholder="Unlimited" value={Number.isNaN(formData.max_usage) ? '' : (formData.max_usage ?? '')} onChange={(e:any) => setFormData({...formData, max_usage: e.target.value === '' ? null : Number(e.target.value)})} className="h-12 rounded-none" /></div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-4 border-t border-foreground/10 dark:border-background/10">
                                        ...
                                        <div className="flex gap-4">
                                            <Button variant="secondary" onClick={() => setIsCreateModalOpen(false)} className="flex-1 h-16 rounded-none text-sm font-bold uppercase tracking-[0.2em]">
                                                Cancel
                                            </Button>
                                            <Button variant="brand" onClick={handleCreate} isLoading={isSubmitting} className="flex-[2] h-16 rounded-none text-sm font-bold uppercase tracking-[0.2em] bg-primary text-background hover:bg-primary/90 dark:bg-background dark:text-foreground dark:hover:bg-background/90 transition-transform">
