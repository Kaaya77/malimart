...
export const BuyerReturns = ({ userId, onContactSeller }: { userId: string, onContactSeller: (sellerId: string, context?: { type: 'order' | 'return' | 'support', id: string, label: string }) => void }) => {
 ...
 <div className="w-12 h-12 rounded-[1.25rem] bg-primary dark:bg-background flex items-center justify-center shadow-2xl shadow-foreground/20 relative group overflow-hidden">
 <div className="absolute inset-0 bg-noise opacity-[0.05] pointer-events-none" />
 <PackageX className="w-6 h-6 text-white relative z-10 group-hover:rotate-12 transition-transform duration-500" />
 ...
 <p className="text-[10px] uppercase tracking-[0.4em] font-black text-foreground/40 ">Resolution Center</p>
 ...
 <div className="flex-1 flex flex-col items-center justify-center text-foreground/20 py-24 text-center bg-white/50 ] rounded-3xl border border-foreground/5 backdrop-blur-sm">
 ...
 <Card key={dispute.id} className="p-10 rounded-3xl bg-card border-foreground/5 shadow-2xl shadow-foreground/5 group hover:border-foreground/20 dark:hover:border-white/20 transition-all duration-700 relative overflow-hidden">
 ...
 <div className="relative z-10 p-6 bg-background ] border border-foreground/5 text-sm text-foreground/60 leading-relaxed font-serif italic mb-8">
 <div className="absolute top-0 left-0 w-1 h-full bg-primary/10 " />
 "{dispute.description}"
 </div>

 <div className="relative z-10 flex items-center justify-between mt-auto pt-6 border-t border-foreground/5 ">
 ...
 <p className="text-[9px] text-foreground/40 font-mono tracking-widest uppercase">{dispute.seller?.phone}</p>
 </div>
 </div>
